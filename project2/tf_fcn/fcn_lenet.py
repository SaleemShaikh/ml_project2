...
class FCNLeNet:
    ...
    def build(self, rgb, train=False, num_classes=20, random_init_fc8=False,
        ...
        self.fc3 = self._fc_layer(self.pool2, "fc3")

        if train:
            self.fc3 = tf.nn.dropout(self.fc3, 0.5)

        self.fc3 = self._fc_layer(self.fc3, "fc7")
        if train:
            self.fc3 = tf.nn.dropout(self.fc3, 0.5)

        if random_init_fc8:
            self.score_fr = self._score_layer(self.fc3, "score_fr",
                                              num_classes)
        else:
            self.score_fr = self._fc_layer(self.fc3, "score_fr",
